...
export class Init1777972664042 implements MigrationInterface {
    name = 'Init1777972664042'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`activity\` (\`id\` varchar(36) NOT NULL, \`action\` varchar(255) NOT NULL, \`description\` text NULL, \`entityType\` varchar(255) NULL, \`entityId\` varchar(255) NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`userId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`user\` (\`id\` varchar(36) NOT NULL, \`fullName\` varchar(255) NOT NULL, \`email\` varchar(255) NOT NULL, \`password\` varchar(255) NOT NULL, \`phone\` varchar(255) NULL, \`role\` enum ('GUARDIAN', 'STAFF', 'ADMIN') NOT NULL DEFAULT 'GUARDIAN', \`isActive\` tinyint NOT NULL DEFAULT 1, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_e12875dfb3b1d92d7d7c5377e2\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`dependent\` (\`id\` varchar(36) NOT NULL, \`fullName\` varchar(255) NOT NULL, \`gender\` enum ('male', 'female') NOT NULL, \`relationship\` enum ('child', 'sibling', 'other') NOT NULL, \`notes\` varchar(255) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`guardianId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`vaccination_record\` (\`id\` varchar(36) NOT NULL, \`administeredDate\` date NULL, \`batchNumber\` varchar(255) NULL, \`clinicalNotes\` varchar(255) NULL, \`status\` enum ('COMPLETED', 'UPCOMING', 'OVERDUE') NOT NULL DEFAULT 'UPCOMING', \`dueDate\` date NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`dependentId\` varchar(36) NULL, \`vaccineId\` varchar(36) NULL, \`administeredById\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
